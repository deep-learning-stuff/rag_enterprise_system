...
def _log_query(
    ...
    gap_id = None if answered else find_or_create_gap(db, query).id
    db.add(
        QueryLog(
            pregunta=query, answered=answered, reason=reason, chunks=chunks, gap_id=gap_id
        )
    )
    db.commit()


def answer_query(db: Session, query: str) -> AnswerData:
    candidates = hybrid_search(db, query)
    if not candidates:
        # Ningún chunk superó el umbral: abstención sin gastar tokens en el LLM.
        _log_query(db, query, [], set(), answered=False, reason="sin_candidatos")
    ...
    try:
        ...
        _log_query(db, query, candidates, set(), answered=False, reason="salida_invalida")
        return AnswerData(answered=False, answer=None, reason="salida_invalida", citations=[])

    if not result.answered or not result.answer:
        _log_query(db, query, candidates, set(), answered=False, reason="llm_abstuvo")
    ...
    if not cited_ids or not cited_ids <= valid_ids:
        ...
        _log_query(db, query, candidates, set(), answered=False, reason="citas_invalidas")
    ...
    _log_query(db, query, candidates, cited_ids, answered=True, reason=None)
